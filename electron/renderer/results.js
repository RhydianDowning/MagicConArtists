...
function renderSections(artistCards, basicLandCards, artistBooths) {
  ...
  for (const artist of sorted) {
    ...
    for (const c of cards) {
      ...
      html += `<div class="card${cls}" data-url="${c.url}" onclick="toggleCardSelect(this)"><img src="${c.image}" loading="lazy" alt="${c.card}"><p>${c.card}<br><small>${c.set} #${c.num}</small></p>${boardLabel}${deckCount}</div>`;
    }
    for (const c of lands) {
      html += `<div class="card basicland" data-url="${c.url}" onclick="toggleCardSelect(this)"><img src="${c.image}" loading="lazy" alt="${c.card}"><p>${c.card}<br><small>${c.set} #${c.num}</small></p></div>`;
    }
    html += `</div></details>`;
  }
